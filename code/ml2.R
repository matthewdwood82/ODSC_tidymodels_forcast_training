...
elec <- elec %>% as_tsibble(index = Date) %>%
...
elec %>% plot_time_series(.date_var = Date, .value = ActivePower)
...
# feasts
elec %>% features(ActivePower, features = guerrero)
...
train <- elec %>% filter_index(. ~ '2010-08-31')
test <- elec %>% filter_index('2010-09-01' ~ .)

train_mods <- train %>% model(Mean = MEAN(ActivePower),
...
# decomposes series into different components

elec %>% model(stl = STL(ActivePower ~ trend(window = 7))) %>%
    components() %>%
    autoplot()

# ETS ---------------------------------------------------------------------
# Error, Trend, Seasonality
# For each, we have 3 options: additive, multiplicative, none

ets_mod <- elec %>%
    model(
        ana = ETS(ActivePower ~ error('A') + trend('N') + season('A')),
        aat = ETS(ActivePower ~ error('A') + trend('A') + season('N')),
...
ets_mod %>% select(-ama) %>% forecast(h = 90) %>% autoplot(elec2010)

ets_auto <-  elec %>% model(auto = ETS(ActivePower))
ets_auto %>% report()
...
elec_diff %>% autoplot(ActivePower) +
    autolayer(elec_diff, PowerDiff, color = 'green') +
    autolayer(elec_diff, PowerDiff2, color = 'blue', alpha = 1 / 3)

elec %>% mutate(PowerLag = difference(ActivePower, lag = 2)) %>% select(ActivePower, PowerLag)
...
# autoregression = regressing y on previous versions of itself, e.g., y_t ~ y_{t-1}
# integrated = created differences
# moving average = regressing y_t on results of previous models
# for more info, see this talk: https://jaredlander.com/content/2019/03/MakingSenseOfAI.html

# THEORETICALLY same as autoregression (but breaks a lot of assumptions of autoregressive models)
elec %>% mutate(Lag=lag(ActivePower)) %>% 
    select(ActivePower, Lag) %>% 
    lm(ActivePower ~ Lag, data = .)

# ARIMA(p = autoregressive, d = differences, q = moving average components)
# ARIMA(p,d,q)(P,D,Q) = caps are seasonal components

arima_323 <- elec %>% 
    model(
        arima=ARIMA(ActivePower ~ pdq(3,2,3) + PDQ(0,0,0))
    )
arima_323 %>% report()
...
arima_mod_3 %>% report()
arima_mod_3 %>% forecast(h=90) %>% autoplot(elec2010)

arima_mod_3 %>% gg_tsresiduals()

# {fable.prophet} will allow you to do {prophet} forecasting using the {fable} framework
# there is a package that will do timeseries xgboost {modeltime}
